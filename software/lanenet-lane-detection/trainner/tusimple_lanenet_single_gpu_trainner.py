...
class LaneNetTusimpleTrainer(object):
    ...
    def __init__(self, cfg):
        ...
        self._binary_prediciton = tf.compat.v1.identity(self._binary_prediciton, name='binary_segmentation_result')

        # define miou
        if self._enable_miou:
            with tf.compat.v1.variable_scope('miou'):
                pred = tf.compat.v1.reshape(self._binary_prediciton, [-1, ])
                gt = tf.compat.v1.reshape(self._input_binary_label_image, [-1, ])
                indices = tf.compat.v1.squeeze(tf.compat.v1.where(tf.compat.v1.less_equal(gt, self._cfg.DATASET.NUM_CLASSES - 1)), 1)
                gt = tf.compat.v1.gather(gt, indices)
                pred = tf.compat.v1.gather(pred, indices)
        ...
        with tf.compat.v1.variable_scope('learning_rate'):
            self._global_step = tf.compat.v1.Variable(1.0, dtype=tf.float32, trainable=False, name='global_step')
            warmup_steps = tf.compat.v1.constant(
                self._warmup_epoches * self._steps_per_epoch, dtype=tf.float32, name='warmup_steps'
            )
            train_steps = tf.compat.v1.constant(
                self._train_epoch_nums * self._steps_per_epoch, dtype=tf.compat.v1.float32, name='train_steps'
            )
            self._learn_rate = tf.compat.v1.cond(
            ...
            self._learn_rate = tf.compat.v1.identity(self._learn_rate, 'lr')
        ...
        with tf.compat.v1.variable_scope(name_or_scope='moving_avg'):
            ...
            moving_ave_op = tf.compat.v1.train.ExponentialMovingAverage(
        ...
        with tf.compat.v1.variable_scope(name_or_scope='train_step'):
            ...
            with tf.compat.v1.control_dependencies(tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS)):
                with tf.compat.v1.control_dependencies([optimize_op, global_step_update]):
                    with tf.compat.v1.control_dependencies([moving_ave_op]):
                        self._train_op = tf.compat.v1.no_op()
